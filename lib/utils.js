...
const defs = require(__dirname + "/defs.js");
...
function writeBuffer(obj, cmdLength, cb) {
	...
	let buff; // Start the offset on the body
	let offset = 16;
	...
	for (const param in defs.cmds[obj.cmdName].params) {
		const paramType = defs.cmds[obj.cmdName].params[param].type;
		const paramSize = paramType.size(obj.params[param]);
	...
	for (const tlvName in obj.tlvs) {
		const tlvValue = obj.tlvs[tlvName].tagValue;
		const tlvId = obj.tlvs[tlvName].tagId;
...
function pduToObj(pdu, stupidNullByte, cb) {
	...
	const retObj = { params: {}, tlvs: {} };

	let command; // 0-15 is the header, so the body starts at 16
	let offset = 16;
...
function objToPdu(obj, cb) {
	...
	const seqNr = parseInt(obj.seqNr);
	...
	calcCmdLength(obj, (err, cmdLength) => {
...
function splitMsg(msg, encoding) {
	...
	const totBitCount = bitCount(msg, resolvedEncoding);
	const logPrefix = topLogPrefix + "splitMsg() - ";
	const msgs = [];
...
function smsDlr(status, cb) {
	...
	sms.session.send(dlrPduObj, false, (err, retPdu) => {
