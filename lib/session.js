...
function smsResp(status, cb) {
	...
	const tasks = [];
	const sms = this; // eslint-disable-line
...
function send(pdu, closeAfterSend, cb) {
	...
	that.on("incomingPduObj" + pduObj.seqNr, incPduObj => {
...
function sendReturn(pdu, status, params, closeAfterSend, cb) {
	...
	utils.pduReturn(pdu, status, params, (err, retPdu) => {
		if (err) {
...
function sendSms(smsOptions, cb) {
	...
	that.send(pduObj, (err, retPduObj) => {
		if (typeof cb === "function") {
...
function sendLongSms(smsOptions, cb) {
	const retPduObjs = [];
	const logPrefix = topLogPrefix + "sendLongSms() - ";
	const encoding = defs.encodings.detect(smsOptions.message); // Set encoding once for all message parts
	const smsIds = [];
	const that = this;
	const msgs = utils.splitMsg(smsOptions.message);
...
function longSms(pduObj) {
	...
	const headerSize = pduObj.params.short_message[2]; // Header size other than first 2 octets
	const csmsReference = pduObj.params.short_message.slice(3, 3 + headerSize - 2); // CSMS Reference starts from 4th octet and length is header size -2 octets
	const partsCount = pduObj.params.short_message[2 + csmsReference.length + 1];
	const longSmsId = pduObj.params.source_addr + "_" + pduObj.params.destination_addr + "_" + csmsReference;
	const partNr = pduObj.params.short_message[2 + csmsReference.length + 2];
	const that = this;
...
function checkLongSmses() {
	...
	for (const smsGroupId in this.longSmses) {
		...
		const udhSize = smsGroup.udhSize;
