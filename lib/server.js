...
const smppUtils = require(__dirname + "/utils");
...
function login(pduObj) {
	...
	if (pduObj.cmdName !== "bind_transceiver" && pduObj.cmdName !== "bind_receiver" && pduObj.cmdName !== "bind_transmitter") {
		...
		smppUtils.pduReturn(pduObj, "ESME_RINVBNDSTS", (err, retPdu) => {
	...
	if (typeof that.options.checkuserpass === "function") {
		return that.options.checkuserpass(pduObj.params.system_id, pduObj.params.password, (err, res, userData) => {
			if (err) return that.closeSocket();
...
function server(options, cb) {
	...
	const serverConn = tlsOrNet.createServer(options, sock => {
