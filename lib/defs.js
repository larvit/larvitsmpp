...
// It seems the order of keys matters in a lot of cases here... no idea how that works
...
encodings.ASCII = { // GSM 03.38
	...
	charCodes: {},
	extChars: {},
	regex: /^[@£$¥èéùìòÇ\nØø\rÅåΔ_ΦΓΛΩΠΨΣΘΞ\x1BÆæßÉ !"#¤%&\'()*+,\-./0-9:;<=>?¡A-ZÄÖÑÜ§¿a-zäöñüà\f^{}\\[~\]|€]*$/, // eslint-disable-line
	...
	encode: function encode(value) {
		...
		value = value.replace(/[\f^{}\\[~\]|€]/g, match => "\x1B" + this.extChars[match]);
	...
	decode: function (value) {
		...
		result = result.replace(/\x1B([\nΛ()\/<=>¡e])/g, (match, ppattern1) => this.extChars[ppattern1]); // eslint-disable-line
...
filters.message = {
	encode: function encode(value) {
		...
		let encoding = encodings.detect(message);
	...
	decode: function decode(value) {
		...
		let udhi = this.esm_class & consts.ESM_CLASS.UDH_INDICATOR;
		let key;
