...
| `reconnect` | on | Re-binds after a drop, an idle timeout, or a stream the library cannot frame, backing off from `minDelay` 1 s to `maxDelay` 30 s and starting over at `minDelay` once a link has lasted `maxDelay`. `{ minDelay, maxDelay }` retunes it; `false` turns it off, so a drop ends the session. `{ fromStart: true }` puts the first connect and bind through the same loop, so a client started while the SMSC is down keeps retrying — including a bind the SMSC refuses — instead of failing. `client()` then resolves once it is bound, and nothing but an aborted `signal` ends that wait: pass `AbortSignal.timeout(ms)` for a deadline. |
